 ...
 * Author: ENG HOSAM
 *
 * Created on August 30, 2023, 9:52 PM
 */
...
Std_ReturnType EUSART_ASYNC_ReadStringBlocking(uint8 *_data, uint16 str_len) {
    ...
    // Read characters until the specified string length is reached
    while (char_counter < str_len) {
        ...
        // Check for newline character (optional - modify as needed)
    ...
    // Check if the required number of characters have been received
    if (char_counter >= str_len) {
        ret = E_OK;  // Indicate successful reception
    }
...
Std_ReturnType EUSART_ASYNC_ReadStringNonBlocking(uint8 *_data, uint16 str_len) {
    ...
    // Check if there is data in the receive buffer and there is space in the output buffer
    while (char_counter < str_len && PIR1bits.RCIF) {
        _data[char_counter] = RCREG;  // Read character from receive buffer
        char_counter++;
    }
...
Std_ReturnType EUSART_ASYNC_WriteByteNonBlocking(uint8 _data) {
    ...
    if (TXSTAbits.TRMT) {
        TXREG = _data;  // Write data to transmit register
        ret = E_OK;
    }
...
Std_ReturnType EUSART_ASYNC_WriteStringNonBlocking(uint8 *_data, uint16 str_len) {
    ...
    // Check if the transmit buffer is empty and there is more data to send
    while (TXSTAbits.TRMT && char_counter < str_len) {
        TXREG = _data[char_counter];  // Write character to transmit register
        char_counter++;
    }
    ...
    if (char_counter >= str_len) {
        char_counter = 0;  // Reset the character counter
        ret = E_NOT_OK;     // Indicate that transmission is complete
    }
