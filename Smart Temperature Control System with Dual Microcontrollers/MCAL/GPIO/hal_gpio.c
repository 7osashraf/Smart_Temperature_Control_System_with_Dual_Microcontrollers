 ...
 * Author: ENG HOSAM
 *
 * Created on July 4, 2023, 1:03 AM
 */
 ...
 * @param _pin_config pointer to the configuration  @ref port_index_t 
...
Std_ReturnType gpio_pin_get_direction_statues(const pin_config_t *_pin_config, direction_t *direction_statues){
    ...
    if(NULL == _pin_config || NULL == direction_statues || (_pin_config->pin > (PORT_PIN_MAX_NUMBER - 1))){
       ...
       *direction_statues = READ_BIT(*tris_registers[_pin_config->port], _pin_config->pin);
    }
