...
static Std_ReturnType Interrupt_INTx_Priority_Init(const interrupt_INTx_t *int_obj){
    ...
    if(NULL == int_obj){
        ...
        switch(int_obj->source){
            case INTERRUPT_EXTERNAL_INT1 : 
                if(INTERRUPT_LOW_PRIORITY == int_obj->priority){ EXT_INT1_LowPriorityySet(); }
                else if(INTERRUPT_HIGH_PRIORITY == int_obj->priority){ EXT_INT1_HighPriorityySet(); }
                else {  /*      Nothing     */}
                ret = E_OK;
                break;
            case INTERRUPT_EXTERNAL_INT2 :
                if(INTERRUPT_LOW_PRIORITY == int_obj->priority){ EXT_INT2_LowPriorityySet();}
                else if(INTERRUPT_HIGH_PRIORITY == int_obj->priority){ EXT_INT2_HighPriorityySet(); }
                else {  /*      Nothing     */}
                ret = E_OK;
                break;
            default : ret = E_NOT_OK;
    ...
    if(NULL == int_obj){
        ...
        switch(int_obj->source){
            case INTERRUPT_EXTERNAL_INT0 : 
                ...
                else if(INTERRUPT_RAISING_EDGE == int_obj->edge){ EXT_INT0_RisingEdgeSet(); }
                else {  /*      Nothing     */}
                ret = E_OK;
                break;
            case INTERRUPT_EXTERNAL_INT1 : 
                if(INTERRUPT_FALLING_EDGE == int_obj->edge){ EXT_INT1_FallingEdgeSet(); }
                else if(INTERRUPT_RAISING_EDGE == int_obj->edge){ EXT_INT1_RisingEdgeSet(); }
                else {  /*      Nothing     */}
                ret = E_OK;
                break;
            case INTERRUPT_EXTERNAL_INT2 :
                if(INTERRUPT_FALLING_EDGE == int_obj->edge){ EXT_INT2_FallingEdgeSet(); }
                else if(INTERRUPT_RAISING_EDGE == int_obj->edge){ EXT_INT2_RisingEdgeSet(); }
                else {  /*      Nothing     */}
                ret = E_OK;
                break;
            default : ret = E_NOT_OK;
